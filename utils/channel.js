...
*/
const channelInsert= async(channelTemp,io,resumeToken, hash_data)=>{
    ...
    try {
        ...
        io.to(user_id).emit("newChannel", {company_id:result.data.company_id,team_id:result.data.team_id,type:result.data.type,channel:result.data.channel,channel_token:resumeToken, hashed_data:hash_data});
        saveChannelEmits({company_id:result.data.company_id,team_id:result.data.team_id,type:result.data.type,channel:result.data.channel,channel_token:resumeToken,hashed_data:hash_data,emit_to:user_id,emit_name:"newChannel"});
    } catch (err) {
        console.log(err);
        sendWebhookError(err, "channelInsert", channelTemp);
    }
    channelInserEmitInCaseOfPublicDirectChannels(result, channelTemp, io, resumeToken, hash_data);
}
...
*   Call saveChannelEmits function to store the event for one minute.
*/
const channelNameUpdate=(channelTemp,io,resumeToken, hash_data)=>{
    try{
        if (channelTemp.type == "direct"){
            ...
            saveChannelEmits({channel:{name:channelTemp.name,_id: channelTemp._id},
                ...
                team_id:channelTemp.team_id,
                company_id:channelTemp.company_id,
                channel_token:resumeToken,emit_to:channelTemp.user_id,emit_name:"channelNameUpdate", hashed_data:hash_data});
        }
        else{
            ...
            saveChannelEmits({channel:{name:channelTemp.name,_id: channelTemp._id},
                ...
                team_id:channelTemp.team_id,
                company_id:channelTemp.company_id,
                channel_token:resumeToken,emit_to:channelTemp._id.toString(),emit_name:"channelNameUpdate",hashed_data:hash_data});
        }
    } catch (error) {
        console.log(error);
...
*/
const channelArchived=async(channelTemp,io,resumeToken, hash_data)=>{
        io.to(channelTemp._id.toString()).emit("channelArchived", {company_id:channelTemp.company_id ,team_id:channelTemp.team_id,type:channelTemp.type,channel:{_id:channelTemp._id.toString(),name:channelTemp.name},channel_token:resumeToken,hashed_data:hash_data});
        saveChannelEmits({company_id:channelTemp.company_id,team_id:channelTemp.team_id,type:channelTemp.type,channel:{_id:channelTemp._id.toString(),name:channelTemp.name},channel_token:resumeToken,emit_to:channelTemp._id.toString(),emit_name:"channelArchived",hashed_data:hash_data});
        let channel_id=channelTemp._id.toString();
        ...
        channelArchiveEmitToSubAdmins(channel_id, channelTemp, resumeToken, io, hash_data);
}
...
*/
const channelUnarchived=async(channelTemp,io,resumeToken, hash_data)=>{
    try {
        let channel_id=channelTemp._id.toString();
        let result;
        if(channelTemp.user_ids.length==0){
            ...
            result =await axios.post(url+"api/channelData", body, configuration);
            createChannelRoom(io,result.data);
            io.to(result.data.channel.creator_id).emit("channelUnArchived", {company_id:result.data.company_id,team_id:result.data.team_id,type:result.data.type,channel:result.data.channel,channel_token:resumeToken,hashed_data:hash_data});
            saveChannelEmits({company_id:result.data.company_id,team_id:result.data.team_id,type:result.data.type,channel:result.data.channel,channel_token:resumeToken,emit_to:result.data.channel.creator_id,emit_name:"channelUnArchived",hashed_data:hash_data});
        }
        else{
            for (let user_id of channelTemp.user_ids) {
                ...
                result =await axios.post(url+"api/channelData", body, configuration);
                createChannelRoom(io,result.data);
                io.to(user_id).emit("channelUnArchived", {company_id:result.data.company_id,team_id:result.data.team_id,type:result.data.type,channel:result.data.channel,channel_token:resumeToken,hashed_data:hash_data});
            };
            saveChannelEmits({company_id:result.data.company_id,team_id:result.data.team_id,type:result.data.type,channel:result.data.channel,channel_token:resumeToken,emit_to:"user_id",emit_name:"channelUnArchived",hashed_data:hash_data});
        }
        channelUnarchiveEmitForPublicPrivateChannels(channel_id,result, io, channelTemp, resumeToken,hash_data);
        chanelUnArchiveEmitToSubAdmins(channel_id, result, channelTemp, resumeToken, io,hash_data);
    } catch (err) {
        console.log(err);
...
const channelArchiveEmitToSubAdmins= async(channel_id, channelTemp, resumeToken, io, hash_data)=>{
        ...
        result1.data.sub_admins.map(async (user_id)=>{
            try {
                if(!channelTemp.user_ids.includes(user_id)){
                    ...
                    io.to(user_id).emit("channelArchived", {company_id:channelTemp.company_id,team_id:channelTemp.team_id,type:channelTemp.type,channel:result_data.data.channel,channel_token:resumeToken,hashed_data:hash_data});
                    saveChannelEmits({company_id:channelTemp.company_id,team_id:channelTemp.team_id,type:channelTemp.type,channel:{_id:channelTemp._id.toString(),name:channelTemp.name},channel_token:resumeToken,emit_to:user_id,emit_name:"channelArchived",hashed_data:hash_data});
                }
            } catch (err) {
                console.log(err);
...
*   Call saveChannelEmits function to store the event for one minute.
*/
const chanelUnArchiveEmitToSubAdmins=async(channel_id, result, channelTemp, resumeToken, io, hash_data)=>{
        let body1 = JSON.stringify({ channel_id, attribute:"channel", operation:"update" });
        let result1 = await axios.post(url+"api/getSubAdmins", body1, configuration);
...
*   Call saveChannelEmits function to store the event for one minute.
*/
const channelUnarchiveEmitForPublicPrivateChannels=async(channel_id, result, io, channelTemp, resumeToken, hash_data)=>{
    try{
        ...
        if(channelTemp.type=='public'){
            ...
            io.to(channelTemp.team_id).emit("publicChannelUnArchived", {company_id:result.data.company_id,team_id:result.data.team_id,type:result.data.type,channel:result.data.channel,channel_token:resumeToken,hashed_data:hash_data});
            saveChannelEmits({company_id:result.data.company_id,team_id:result.data.team_id,type:result.data.type,channel:result.data.channel,channel_token:resumeToken,emit_to:channelTemp.team_id,emit_name:"publicChannelUnArchived",hashed_data:hash_data});
        }
...
*   Call saveChannelEmits function to store the event for one minute.
*/
const channelInserEmitInCaseOfPublicDirectChannels=(result, channelTemp, io, resumeToken, hash_data)=>{
    try{
        ...
        if(channelTemp.type=='public'){
            ...
            io.to(channelTemp.team_id).emit("newPublicChannel", {company_id:resultClone.company_id,team_id:resultClone.team_id,type:resultClone.type,channel:resultClone.channel,channel_token:resumeToken,hashed_data:hash_data});
            saveChannelEmits({company_id:resultClone.company_id,team_id:resultClone.team_id,type:resultClone.type,channel:resultClone.channel,channel_token:resumeToken,emit_to:channelTemp.team_id,emit_name:"newPublicChannel",hashed_data:hash_data});
        }
            ...
            io.to(channelTemp.user_id).emit("newDirectChannel", {company_id:resultClone.company_id,team_id:resultClone.team_id,type:resultClone.type,channel:resultClone.channel,channel_token:resumeToken,hashed_data:hash_data});
            saveChannelEmits({company_id:resultClone.company_id,team_id:resultClone.team_id,type:resultClone.type,channel:resultClone.channel,channel_token:resumeToken,emit_to:channelTemp.user_id,emit_name:"newDirectChannel",hashed_data:hash_data});
        }
    } catch (error) {
        console.log(error);
