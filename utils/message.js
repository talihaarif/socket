...
const queryMessageUpdate= async (io,messageTemp,messageUpdateCheck,ids,update_message_emit_name,hash,channel_id)=>{
    ...
    else if (messageUpdateCheck.is_read || messageUpdateCheck.child_read || messageUpdateCheck.pinned_by || messageUpdateCheck.replied_created_at || messageUpdateCheck.replied_sender_id || (Object.keys(messageUpdateCheck)[0].search('reactions') >= 0 )|| (messageUpdateCheck.updated_at && Object.keys(messageUpdateCheck).length == 1)) {
        return 0;
    }
    else {
        await messageEmit(io,messageTemp.channel_id,update_message_emit_name,messageTemp,ids,hash,channel_id);
